...
class DataEncryptionStandard {
	...
	private function generateKey($key){
		...
		$this->log['key']['hexKey'] = $this->stringToHex($key); 
		$binKey = $this->log['key']['binKey'] = $this->stringToHexToBin($key);
		$keyPC1 = '';
		...
		for($i = 1; $i <= 16; $i++) {
			...
			for($shiftIndex = 1; $shiftIndex <= $this->r[$i]; $shiftIndex++) {
				$shiftC = $this->leftShift($shiftC); 
				$shiftD = $this->leftShift($shiftD); 
			}
			$C[$i] = $shiftC;
			$D[$i] = $shiftD;
		}
	...
	public function encrypt($message, $key){
		if(strlen($message) == 8 && strlen($key) == 8) {
			...
			echo 'Plaintext dan Key Harus 8 Karakter.';
		}
	}
